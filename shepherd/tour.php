...
class cmplz_tour {
	...
	function __construct() {
		...
		add_action( 'wp_ajax_cmplz_cancel_tour', array( $this, 'listen_for_cancel_tour' ) );
	...
	public function enqueue_assets( $hook ) {
		...
		if ( get_site_option( 'wpdef_tour_started' ) ) {
			...
			wp_register_style( 'cmplz-shepherd',
				trailingslashit( $this->url )
				. "css/shepherd-theme-arrows.min.css", "",
				$this->version );
			wp_enqueue_style( 'cmplz-shepherd' );

			wp_register_style( 'cmplz-shepherd-tour',
				trailingslashit( $this->url ) . "css/cmplz-tour.min.css", "",
				$this->version );
			...
			$steps = array(
				0 => array(
					'title'  => __( 'Welcome to Bob the Linkbuilder', 'definitions' ),
					'text'   => __( "You can find the settings of Bob the Linkbuilder under every posts. To get a headstart, have a look at our documentation.",
						'definitions' ),
					'start_link'   => admin_url( "edit.php" ),
					'documentation_link'   => "https://really-simple-plugins.com/bob-the-linkbuilder/documentation",
				),
			);
			...
			wp_localize_script( 'cmplz-shepherd-tour', 'cmplz_tour',
				array(
					...
					'configure'      => __( "Configure", "complianz-gdpr" ),
