...
class rspdef_tour {
	...
	function __construct() {
		...
		$this->url     = RSPDEF_URL . '/shepherd';
		$this->version = RSPDEF_VERSION;

		add_action( 'wp_ajax_rspdef_cancel_tour', array( $this, 'listen_for_cancel_tour' ) );
	...
	public function enqueue_assets( $hook ) {
		...
		if ( get_site_option( 'rspdef_tour_started' ) ) {
			...
			$steps = apply_filters( 'rspdef_shepherd_steps', $steps );
			wp_localize_script( 'rspdef-shepherd-tour', 'rspdef_tour',
				array(
	...
	public function listen_for_cancel_tour() {
		...
		if ( ! isset($_POST['token']) || ! wp_verify_nonce($_POST['token'], 'rspdef_tour_nonce') ) {
			return;
		}
		update_site_option( 'rspdef_tour_started', false );
		update_site_option( 'rspdef_tour_shown_once', true );
	}


	public function restart_tour() {

		if ( ! isset($_POST['rspdef_restart_tour']) ) {
			return;
		}
