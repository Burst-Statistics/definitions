...
if ( ! class_exists( "rspdef_review" ) ) {
	class rspdef_review {
		...
		public function init(){
			...
			$definitions_count = get_transient('rspdef_definitions_count');
			if ( !$definitions_count ) {
				$definitions_count = wp_count_terms( $args );
				set_transient('rspdef_definitions_count', $definitions_count, DAY_IN_SECONDS);
			}
			...
			$this->show_by_count = ($definitions_count > 5 && $found_in_posts_count>5) && get_option( 'rspdef_activation_time' ) < strtotime( "-2 weeks" );
            $show_by_time = get_option( 'rspdef_activation_time' ) < strtotime( "-1 month" );
			//uncomment for testing
//			update_option('rspdef_review_notice_shown', false);
//			update_option( 'rspdef_activation_time', strtotime( "-2 month" ) );
			//show review notice, only to free users
			if ( ! defined( "rspdef_premium" ) && ! is_multisite() ) {
				if ( ! get_option( 'rspdef_review_notice_shown' )
				     && get_option( 'rspdef_activation_time' )
				...
				if ( ! get_option( 'rspdef_activation_time' ) ) {
		...
		public function show_leave_review_notice() {
			if (isset( $_GET['rspdef_dismiss_review'] ) ) return;
			...
			<style>
				...
				.rspdef-container .dashicons {
					margin-left: 10px;
					margin-right: 5px;
				}

				.rspdef-review-image img {
					margin-top: 0.5em;
				}
			...
			<div id="message"
			     class="updated fade notice is-dismissible rspdef-review really-simple-plugins"
			     style="border-left:4px solid #333">
				<div class="rspdef-container">
					<div class="rspdef-review-image">
		...
		public function insert_dismiss_review() {
			$ajax_nonce = wp_create_nonce( "rspdef_dismiss_review" );
			?>
			<script type='text/javascript'>
                jQuery(document).ready(function ($) {
                    $(".rspdef-review.notice.is-dismissible").on("click", ".notice-dismiss", function (event) {
                        rsssl_dismiss_review('dismiss');
                    });
                    $(".rspdef-review.notice.is-dismissible").on("click", "#maybe-later", function (event) {
                        rsssl_dismiss_review('later');
                        $(this).closest('.rspdef-review').remove();
                    });
                    $(".rspdef-review.notice.is-dismissible").on("click", ".review-dismiss", function (event) {
                        rsssl_dismiss_review('dismiss');
                        $(this).closest('.rspdef-review').remove();
                    });
		...
		public function dismiss_review_notice_callback() {
			...
			if ( $type === 'dismiss' ) {
				update_option( 'rspdef_review_notice_shown', true );
			}
			if ( $type === 'later' ) {
		...
		public function process_get_review_dismiss(){
			if (isset( $_GET['rspdef_dismiss_review'] ) ){
				update_option( 'rspdef_review_notice_shown', true );
			}
		}
	}
